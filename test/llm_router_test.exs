defmodule Pado.LLMRouterTest do
  ...
  test "알 수 없는 API는 어댑터 없음 오류를 반환한다" do
    model = %Model{id: "dummy", provider: :dummy, api: :unknown_api}
    ctx = Context.new()

    assert {:error, {:no_adapter, :unknown_api}} = LLMRouter.stream_text(model, ctx)
    assert {:error, {:no_adapter, :unknown_api}} = LLMRouter.generate_text(model, ctx)
  end
  ...
  test "OpenAI Codex 어댑터는 크레덴셜을 사전 검증한다" do
    ...
    assert {:error, :missing_credentials} = LLMRouter.stream_text(model, ctx)
    ...
    assert {:error, {:wrong_provider_credentials, :other_provider}} =
             LLMRouter.stream_text(model, ctx, credentials: wrong_provider)

    missing_account_id = Credentials.build(:openai_codex, "access", "refresh", 3600)

    assert {:error, :missing_account_id} =
             LLMRouter.stream_text(model, ctx, credentials: missing_account_id)
  end
end
