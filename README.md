...
config :pado, credentials: %{
  openai_codex:
    {Pado.LLM.Credential.FileLoader, Path.expand("~/.config/pado/openai.json")}
}
```
...
```bash
$ mix pado.llm.login --output ~/.config/pado/openai.json
```

매핑도 `--output`도 없으면 stdout으로 JSON을 뱉어 셸 redirect로 처리해도 된다:

```bash
$ mix pado.llm.login > some.json
```
...
```elixir
alias Pado.LLM
alias Pado.LLM.Catalog.OpenAICodex, as: OpenAICodexCatalog
...
{:ok, stream} = LLM.stream(model, ctx, creds, "session-1", reasoning_effort: :low)
...
{:ok, creds} = Pado.LLM.Credential.OAuth.OpenAICodex.login(callbacks)
```
