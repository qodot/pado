defmodule Pado.Agent.Session.Codec do
  ...
  def session_to_map(%Session{} = session) do
    %{
      ...
      "entries" => Enum.map(session.entries, &entry_to_map/1)
    }
  end

  @spec session_from_map(map()) :: {:ok, Session.t()} | {:error, term()}
  def session_from_map(%{"type" => "session"} = map) do
    with {:ok, provider} <- decode_provider(map["provider"]),
         {:ok, reasoning_effort} <- decode_reasoning_effort(map["reasoning_effort"]),
  ...
  def session_from_map(map), do: {:error, {:invalid_session_map, map}}

  @spec entry_to_map(Entry.t()) :: map()
  def entry_to_map(%Entry{} = entry) do
    %{
      "type" => "entry",
      "id" => entry.id,
      "seq" => entry.seq,
  ...
  @spec entry_from_map(map()) :: {:ok, Entry.t()} | {:error, term()}
  def entry_from_map(%{"type" => "entry"} = map) do
  ...
  defp decode_reasoning_effort("none"), do: {:ok, :none}
  defp decode_reasoning_effort("low"), do: {:ok, :low}
  defp decode_reasoning_effort("medium"), do: {:ok, :medium}
  defp decode_reasoning_effort("high"), do: {:ok, :high}
  defp decode_reasoning_effort("xhigh"), do: {:ok, :xhigh}
  defp decode_reasoning_effort(value), do: {:error, {:unknown_reasoning_effort, value}}
  ...
  defp decode_payload(:assistant, map) when is_map(map) do
    with {:ok, content} <- decode_content(map["content"]),
         ...
         {:ok, provider} <- decode_provider(map["provider"]),
         {:ok, timestamp} <- decode_datetime(map["timestamp"]) do
      {:ok,
       %Assistant{
         content: content,
  ...
  defp decode_payload(:model_change, map) when is_map(map) do
    with {:ok, provider} <- decode_provider(map["provider"]),
         {:ok, reasoning_effort} <- decode_reasoning_effort(map["reasoning_effort"]) do
      {:ok,
       %ModelChange{
         provider: provider,
         from: map["from"],
