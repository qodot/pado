...
장기 실행되는 AI 에이전트를 만드는 데 필요한 계층을 쌓아 갑니다. 각 계층은 `Pado.*`
네임스페이스 아래 독립된 하위 시스템으로 공존합니다.

> **상태: 초기 단계.** 현재는 `Pado.LLM`(LLM 프로바이더 API 클라이언트)만
> 구현 중입니다. OpenAI Codex(ChatGPT Plus/Pro 구독) OAuth 로그인, 크레덴셜 모델,
> `/codex/responses` 스트리밍 호출, SSE 파싱, 정규화 이벤트 매핑까지 1차 구현되어
> 있습니다. 에이전트 루프와 웹 UI는 아직 없습니다.

---

## 참고한 프로젝트

Pado의 설계는 두 프로젝트에서 **강력한 영향**을 받았습니다. 개념 계층 대부분을
그대로 빌려 왔고, 그 위에서 Elixir/OTP와 서버사이드 환경에 맞게 다시 빚었습니다.

### [Pi (`@mariozechner/pi-mono`)](https://github.com/badlogic/pi-mono)

Mario Zechner의 TypeScript 기반 터미널 코딩 에이전트입니다.
특히 다음 패키지들의 설계를 정독하고 가져왔습니다.

- **`pi-ai`** — LLM 프로바이더 어댑터 레지스트리, OAuth 구독 로그인 플로우,
  스트리밍 이벤트 추상. `Pado.LLM`의 거의 모든 추상이 여기서 왔습니다.
  OpenAI Codex OAuth 플로우(엔드포인트, 비표준 파라미터, JWT 클레임 구조)도
  pi-ai의 `utils/oauth/openai-codex.ts`를 사실상 포트한 것입니다.
- **`pi-agent-core`** — 에이전트 루프(턴 반복, 도구 실행, steer/followUp
  메시지 큐잉, 세분화된 `AgentEvent` 스트림). 향후 `Pado.Agent`가 참조할 설계입니다.

### [Jido (`agentjido/jido`, `jido_ai`, `jido_action`)](https://github.com/agentjido/jido)

Mike Hostetler의 Elixir 자율 에이전트 프레임워크입니다.
OTP 위에서 상태 있는 에이전트를 **불변 `cmd/2` + Directive + Signal 라우팅 +
AgentServer**로 정렬한 모델을 참고합니다. 특히 다음 부분을 봅니다.

- **`jido`(코어)** — 감독트리 통합, 파티션 멀티테넌시, 스케줄러, 플러그인.
  향후 `Pado.Agent`의 런타임 기반입니다.
...
- **[`req_llm`](https://hex.pm/packages/req_llm)** — Elixir용 멀티 프로바이더
  ...
  `req_llm` 어댑터로 위임할 가능성을 열어 둡니다.

> 위 프로젝트들은 각자의 철학·실행 모델이 다르고, 이 저장소에도 그 차이의 흔적이
> 그대로 녹아 있습니다. Pado는 두 설계의 복제가 아니라, **"Pi의 API 모양 + Jido의
> OTP 런타임"** 을 이식하려는 시도입니다.
...
1. **실행 환경은 서버사이드입니다.** 터미널 CLI가 아니라 장기 실행되는 Elixir
   애플리케이션 안에서 에이전트가 돕니다.
2. **멀티 에이전트·멀티테넌시가 자연스러워야 합니다.** 세션별로 격리된 프로세스,
   감독트리, 파티션이 필요하면 Jido의 원시요소를 그대로 씁니다.
3. **사용자 상호작용은 주로 웹 UI**(Phoenix LiveView)입니다. 따라서 이벤트 스트림은
   PubSub/Channel과 자연스럽게 맞물려야 합니다.
4. **LLM 프로바이더·OAuth 세부사항은 한 번 쓰고 잊고 싶습니다.** Pi의 pi-ai가 이미
   만들어 둔 것을 Elixir 관용구로 옮기면 됩니다.

이 조합은 Pi에도 Jido에도 그대로 존재하지 않습니다. 둘의 공통 부분집합에 OTP 운영성
위에서 서버사이드 UX를 얹는 것이 Pado가 메우려는 빈칸입니다.

---

## 생태계 구조

`Pado`는 생태계 진입점 모듈이고, 실제 기능은 하위 시스템에 들어갑니다.
...
패키지로 분리할 수 있도록 네임스페이스부터 격리해 둡니다.
...
- **라이브러리 핵심은 어떤 영속 저장소도 소유하지 않습니다.** `login/2`는
  `%Credentials{}`를 반환할 뿐이고, 저장·갱신·로테이션 관리는 호출자(서비스 앱)의
  책임입니다. Pi가 `pi-coding-agent`의 `AuthStorage`에 그 책임을 분리해 둔 것과
  같은 원칙입니다. 흔한 JSON 파일 저장소는 `Pado.LLM.Credential.FileLoader`에
  helper로 격리되어 있고, 호출자가 config에 매핑만 선언하면 자동 저장/조회됩니다.
  DB/Vault/KMS 같은 다른 저장소는 호출자가 같은 인터페이스(`load/1`, `save/2`)를
  가진 모듈을 만들어 매핑에 등록하면 됩니다.
- **콜백 서버는 라이브러리 안에 내장**되어 있습니다. OpenAI의 `redirect_uri`가
  `http://localhost:1455/auth/callback`으로 서버에 등록되어 있어 우회할 수 없는
  프로토콜 상수이기 때문입니다. 다만 UI·저장·프롬프트는 전부 호출자가 주입하는
  콜백 맵으로 분리됩니다.
- **`:bandit`/`:plug`는 선택 의존성입니다.** 이미 크레덴셜을 가진 서비스(예:
  Vault에서 로드)는 콜백 서버를 깔 필요가 없습니다.
- **`Pado.LLM`는 도구를 실행하지 않습니다.** 도구 목록을 모델에 알려주고,
  모델이 요청한 `tool_call`을 정규화된 Assistant 메시지로 돌려주는 데까지만
  책임집니다. 실제 도구 실행과 다음 턴 반복은 향후 `Pado.Agent`의 책임입니다.
- **스트리밍은 이벤트 Enumerable로 노출합니다.** 상위 계층은 토큰 델타,
  tool_call 델타, 종료 이벤트를 그대로 소비합니다.

---

## 로드맵

구체적 약속이 아니라 방향성 스케치입니다.
...
- **Pado.Agent** — `use Jido.Agent` 위에 Pi 스타일 에이전트 루프(턴 반복,
  도구 실행, steer/followUp 큐, 세분화된 이벤트 스트림)를 얇게 얹습니다.
  `jido_ai`의 ReAct 전략을 그대로 쓰기보다 루프 의사결정을 소유하는 쪽으로 기웁니다.
...
아직 Hex에 게시되지 않았습니다. path 또는 git 의존성으로 사용합니다.
...
그대로 따릅니다. 같은 구조를 따르므로, 양쪽에서 만든 크레덴셜은
`Credentials.from_map/1`의 `expires_at` 변환을 거치면 상호 호환됩니다.

Pi와 Jido가 없었다면 Pado는 지금의 설계에 훨씬 오래 걸려 도달했을 것입니다.
두 프로젝트 저자들에게 감사를 표합니다.
